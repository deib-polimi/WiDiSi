...
/**
 * The Class GraphFactoryM.
 */
public class GraphFactoryM {
...
// Naser
// I added a method to this class to perform the graph wiring based on the coordination
 ...
 * @param radio_range the radio_range
 * @return the graph
 */
// Naser: Wiring based on the coordination
public static Graph wireCordXY(Graph g, int coordPid, double radio_range) {
	 for (int i = 0; i < Network.size(); i++) {
		 for(int j=0; j< Network.size(); j++){
			 if (i==j) continue; 
			 if((distance(Network.get(i), Network.get(j), coordPid)< radio_range) && (!g.isEdge(i, j))){
				 g.setEdge(i, j);
			 }
		 }
	 }
